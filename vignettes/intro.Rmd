...
This `data.frame` has information on 8 individuals with these columns:
  ...
  * `bv1`: Breeding value for trait 1, and
  
  * `bv2`: Breeding value for trait 2.
...
ped <- AlphaPart.ped[, c("IId", "FId", "MId")]
ped$FId[ped$FId == ""] <- NA
ped$MId[ped$MId == ""] <- NA
ped[] <- lapply(ped, as.character)
ped <- pedigree(label = ped$IId,
                sire = ped$FId,
                dam = ped$MId)
getT(ped)
```
...
You can partition multiple traits by specifying a vector of variables, say `colBV = c("bv1", "bv2")`. This multiple trait option can also serve to partition samples from a posterior distribution to quantify uncertainty - see the [samples](samples.html) vignette.
